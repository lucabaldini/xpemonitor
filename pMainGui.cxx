  ...
  : TGMainFrame(parent, width, height)
{
  ...
  string tarray1[] = {"EvNum:", "BufferId:","WindowSize:","TimeStamp (s):","Loops/s:"};
  ...
  fTab = new TGTab(m_canvasFrame, 900, 600);
  ...
  TGGroupFrame *fLeftVerticalFrameS = new TGGroupFrame(fLeftFrame, "Info", kVerticalFrame);  
  status = new TGStatusBar(fLeftVerticalFrameS,100,100);
  status->SetParts(2);
  fLeftVerticalFrameS->AddFrame(status, new TGLayoutHints(kLHintsLeft| kLHintsExpandX,2,2,0,5));
  status2 = new TGStatusBar(fLeftVerticalFrameS,100,100);
  fLeftVerticalFrameS->AddFrame(status2, new TGLayoutHints(kLHintsLeft| kLHintsExpandX,2,2,0,5));


  fLeftFrame->AddFrame(fLeftVerticalFrameS, new TGLayoutHints(kLHintsLeft| kLHintsExpandX ,2,0,4,2));
  feCanvas1 = new TRootEmbeddedCanvas("ec1", fRightFrame, 750, 650);
  fRightFrame->AddFrame(feCanvas1, new TGLayoutHints(kLHintsExpandX | kLHintsExpandY, 5, 5, 5, 5));
  ...
  string tarray3[] = {"BufferId:","WindowSize:","TimeStamp (s):","Loops/s:"};
  ...
  m_LMcanvas->Connect("ProcessedEvent(int,int,int,TObject*)", "pMainGui", 
...
void pMainGui::ExecEvent(int event,int x,int y,TObject* selected){
 int xb, yb, content;
  ...
  sprintf(atext, "x: %d", (int)xc); 
  status->SetText(atext,0); 
  sprintf(atext, "y: %d", (int)yc);
  status->SetText(atext,1);

  content = (int)yc;

  if(((TH1*)gPad->GetPrimitive("CumulativeHisto"))){
    xb = (cumulativeHisto->GetXaxis())->FindFixBin(xc); 
    yb = (cumulativeHisto->GetYaxis())->FindFixBin(yc); 
    content = (int)cumulativeHisto->GetBinContent(xb,yb);
   }
  if(((TH1*)gPad->GetPrimitive("RawEv2D"))){
    xb = (rawEv2D->GetXaxis())->FindFixBin(xc); 
    yb = (rawEv2D->GetYaxis())->FindFixBin(yc); 
    content = (int)rawEv2D->GetBinContent(xb,yb);
  }
  sprintf(atext,"Content: %d",content);
  status2->SetText(atext,0);	 
 }
...
void pMainGui::Stop(){
  ...
  if(LMon==true) for(int i=0;i<2;i++)((TGTextEntry*)inTab1[i])->SetState(true);
   
  if(LMon==false) for(int i=0;i<6;i++)((TGTextEntry*)inTab2[i])->SetState(true);
 }
...
void pMainGui::SetupHistograms()
{
  ...
  if (LMon == true){
    if(histosLM == false){
      ...
      cumulativeHisto = new TH2F("CumulativeHisto", "Raw Hit Map ", 352, 0, 352, 300, 0, 300 );
      cumulativeHisto->SetStats(false);
      histosLM = true;
    }
    DrawHistos();
  }
...
{  
  ...
  if (!gSystem->AccessPathName("info.dat", kFileExists)) {
    info.open("info.dat",ios::in);
    for(int i=0; i<6; i++) info>>Dummy;
    info>>HitDisplayFreq;
  }
  else HitDisplayFreq = 100; // display 2D raw map every 200 ev
  ReadSettings();
  eventCnt = 0;
      ...
      for (int iy=ymin; iy<=ymax; iy++)
	{
	  for (int ix=xmin; ix<=xmax; ix++)
	    {    
	      ...
	      //cumulativeHisto->Fill((float)iy ,(float)ix ,RawChannelData[chan]);
	      if(signal>=threshold) {
		cumulativeHisto->Fill((float)iy ,(float)ix ,RawChannelData[chan]);
...
void pMainGui::FullMon(){
  ...
  while(isRunning)
	...
	continue;
      ...
      numPix = (xmax+1- xmin)*(ymax+1- ymin);
      tt.Form("%d",numPix);
      ...
      buffId1 = buffId;
      deltastamp = timestamp1 - timestamp0;
      int dbuff = buffId1-buffId0;
      if(deltastamp)rate = dbuff/deltastamp;
      //else rate = 0;
      ...
      tt.Form("%4.2f",rate);
      if(!dbuff) tt = "---";
      ((TGTextEntry*)outTab2[3])->Clear();
      ((TGTextBuffer*)outBufTab2[3])->AddText(0,tt);
...
void pMainGui::SetHitBkBlack(){
  cumulativeHisto->GetXaxis()->SetLabelColor(kWhite);
  cumulativeHisto->GetXaxis()->SetTitleColor(kWhite);
  cumulativeHisto->GetXaxis()->SetAxisColor(kWhite);
  cumulativeHisto->GetYaxis()->SetLabelColor(kWhite);
  cumulativeHisto->GetYaxis()->SetTitleColor(kWhite);
  cumulativeHisto->GetYaxis()->SetAxisColor(kWhite);
  p0->cd(2);
...
void pMainGui::SetHitBkWhite(){
  cumulativeHisto->GetXaxis()->SetLabelColor(kBlack);
  cumulativeHisto->GetXaxis()->SetTitleColor(kBlack);
  cumulativeHisto->GetXaxis()->SetAxisColor(kBlack);
  cumulativeHisto->GetYaxis()->SetLabelColor(kBlack);
  cumulativeHisto->GetYaxis()->SetTitleColor(kBlack);
  cumulativeHisto->GetYaxis()->SetAxisColor(kBlack);
  p0->cd(2);
